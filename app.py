"""
Main entry point for VTuber face tracking system.
"""
import sys
import os
import argparse
...
class VTuberTrackerApp:
    ...
    def initialize_components(self):
        ...
        try:
            # Initialize camera - retry mechanism
            initial_camera_index = self.config['camera']['default_camera_index']
            max_retries = 3
            for attempt in range(max_retries):
                try:
                    self.camera = CameraCapture(
                        camera_index=initial_camera_index,
                    ...
                    if test_frame is not None:
                        ...
                        self.logger.warning(f"Camera test failed on attempt {attempt+1}, retrying...")
                        if self.camera:
                            self.camera.release()
                        import time
            ...
            if self.config.get('virtual_camera', {}).get('enabled', False):
                ...
                if self.virtual_camera:
                    ...
                    self.logger.info("Virtual camera initialized and enabled")
                else:
                    self.logger.warning("Virtual camera not available, continuing without it")
    ...
    def run_tracking_loop(self):
        ...
        if test_frame is None:
            self.logger.error("Cannot get frames from camera. Please check your camera connection.")
            # Continue running but with a warning - useful for testing without camera
            self.logger.info("Running in test mode without camera. Using simulated data.")
        else:
            self.logger.info("Camera connection confirmed. Starting tracking loop...")
        ...
        try:
            while self.is_running:
                ...
                if frame is None:
                    # Instead of just continuing, we can implement a retry or wait mechanism
                    self.logger.warning("Failed to get frame from camera, waiting...")
                    import time
                ...
                if self.virtual_camera:
                    # We should send the original frame with face landmarks drawn for the virtual camera
                    frame_with_landmarks = self.face_tracker.draw_landmarks(frame.copy(), self.face_tracker.get_landmarks(frame))
    ...
    def run_gui(self):
        ...
        try:
            ...
            # Still return True to indicate that the app should continue
            return True
